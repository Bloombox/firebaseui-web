...
firebaseui.auth.widget.handler.handlePasswordRecovery = function(
  ...
  // Render the UI.
  const component = new firebaseui.auth.ui.page.PasswordRecovery(
      // On submit.
      function() {
...
firebaseui.auth.widget.handler.onPasswordRecoverySubmit_ =
  ...
  const emailSentSuccessCallback = function() {
    ...
    // password reset).
    const noticeComponent = new firebaseui.auth.ui.page.PasswordRecoveryEmailSent(
        /** @type {string} */ (email),
        function() {
  ...
  const emailSentFailedCallback = function(error) {
